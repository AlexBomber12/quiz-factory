...
describe("GET /api/admin/analytics/revenue", () => {
  ...
  it("returns the expected response shape", async () => {
    ...
    const response = await GET(
      new Request("http://localhost:3000/api/admin/analytics/revenue?start=2026-02-01&end=2026-02-07")
    );
    ...
    expect(payload).toMatchObject({
      ...
      by_offer: expect.any(Array),
      by_tenant: expect.any(Array),
      by_test: expect.any(Array),
      reconciliation: {
    ...
    expect(payload.kpis[0]).toMatchObject({
      key: expect.any(String),
      label: expect.any(String),
      value: expect.any(Number)
    });
    expect(payload.by_offer[0]).toMatchObject({

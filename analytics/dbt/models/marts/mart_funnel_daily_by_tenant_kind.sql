{{
  config(
    ...
    unique_key=['date', 'tenant_kind', 'locale', 'channel_key'],
    partition_by={'field': 'date', 'data_type': 'date'},
    cluster_by=['tenant_kind', 'locale', 'channel_key']
  )
}}
...
with events_base as (
  select
    ...
    case
      when lower(nullif(trim(tenant_kind), '')) in ('hub', 'niche') then
        lower(nullif(trim(tenant_kind), ''))
      else 'hub'
    end as tenant_kind,
    locale,
    distinct_id,
...
aggregated as (
  ...
  from attributed
  group by date, tenant_kind, locale, channel_key
)
